...
def find_new_bikes(self):
    ...
    if len(res) > 0:
        for item in res:
            if (item[0] != 'Unknown') and (item[0] not in list(self.all_bike_ids.keys())):
                self.add_new_bike(*item)
            else:
                self.add_unknown_bike(item)
    elif item[0] not in list(self.all_bike_ids.keys()):
        self.message_box.setText('No bikes found in rides! Try updating rides first.')
    else:
        self.message_box.setText('No new bikes found.')
