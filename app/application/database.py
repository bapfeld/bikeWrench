...
def get_rider_info():
    ...
    try:
        ...
        tot_climb = 0
    max_speed, avg_speed, tot_dist, tot_climb = convert_rider_info(
        units, max_speed, avg_speed, tot_dist, tot_climb
    )
    return (
        rider_name,
        units,
...
def update_rider(current_nm, nm, units):
    (
        ...
        .update({Riders.name: nm, Riders.units: units})
    )
    db.session.commit()
...
def update_part(p_id, p_type, added, brand, price, weight, size, model, virtual):
    (
        db.session.query(Parts)
...
def get_all_bike_parts(current_bike):
    current_bike_parts_list = (
        Parts.query.filter(Parts.bike == current_bike)
...
def get_ride_data_for_bike(bike_id, units, start_date=None, end_date=None):
    query = db.session.query(
        func.sum(Rides.distance).label("dist"),
...
def get_ride_data_for_part(bike_id, units, early_date, late_date):
    query = db.session.query(
        ...
        and_(Rides.bike == bike_id, Rides.date >= early_date, Rides.date <= late_date)
    )
    q_virt = query.filter(Rides.tp == "VirtualRide")
    res_all = query.first()
    res_virt = q_virt.first()
...
def add_new_bike(bike_id, bike_name, bike_color, bike_purchase, bike_price, bike_mfg):
    new_bike = Bikes(
        bike_id=bike_id,
        name=bike_name,
        color=bike_color,
...
def add_maintenance(part_id, work, dt):
    new_maint = Maintenance(part=part_id, work=work, date=dt)
...
def find_new_bikes():
    ...
    if len(new_bikes) > 0:
        b_list = [Bikes(bike_id=b, name=f"tmp_nm_{b}") for b in new_bikes]
...
def get_part_details(part_id):
    ...
    b_id, b_nm = (
        db.session.query(Bikes.bike_id, Bikes.name)
