...
- [X] How to deal with alphanumeric ids and something intelligible?
...
- [X] Printing numbers should be rounded to logical places
- [X] How to identify which bike is which per strava ids
- [X] Need to consider what happens if this is run without the internet
- [X] Parts function needs to get the original part date for all stats option
- [X] Throws an error if no input is supplied when prompted
